 ...
 * @author Nidebruyn
 */
public abstract class Physics2DEditor extends AbstractScreen implements PickListener, ActionListener {
     ...
     * @return
     */
    protected abstract Physics2DGame initGame();
    ...
    protected void init() {
        ...
        createTileButton(Physics2DGame.TYPE_TERRAIN, "Resources/button_terrain.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_STATIC, "Resources/button_static.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_ENEMY, "Resources/button_enemy.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_OBSTACLE, "Resources/button_obstacle.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_VEGETATION, "Resources/button_veg.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_PICKUP, "Resources/button_pickup.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_START, "Resources/button_start.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_END, "Resources/button_end.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_SKY, "Resources/button_sky.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_BACK_LAYER1, "Resources/button_sky.png", toolsPanel, tilesListener);
        createTileButton(Physics2DGame.TYPE_BACK_LAYER2, "Resources/button_sky.png", toolsPanel, tilesListener);
    ...
    protected void initInput() {
        log("Init input");
        if (!inputManager.hasMapping(KEYBOARD_ADD)) {
            ...
            inputManager.addListener(this, new String[]{KEYBOARD_ADD, KEYBOARD_DEL});
        }
...
//        marker.setLocalTranslation(0, 0, 0);
    ...
    protected void load() {
        ...
        index = 0;
        selectedList = game.getTypeList(Physics2DGame.TYPE_TERRAIN);
                    ...
                    if (KEYBOARD_ADD.equals(name)) {
                        if (isPressed) {
                            if (selectedSprite != null && selectedList.size() > 0) {
                                ...
                                Tile tile = new Tile(pos.x, pos.y, sprite.getWorldTranslation().z, selectedSprite.getLocalRotation().toAngleAxis(Vector3f.UNIT_Z), selectedList.get(index));
...
//                    }
    ...
    public void picked(PickEvent pickEvent, float tpf) {
        if (isActive() && game != null) {
            ...
            //Update the marker
            marker.setLocalTranslation(pickEvent.getContactPoint().x, pickEvent.getContactPoint().y, 1);
            ...
            if (!pickEvent.isKeyDown() && pickEvent.isLeftButton() && !descriptionDialog.isVisible() && pickEvent.getContactPoint() != null
                ...
                if (!deleteActive && pickEvent.getContactObject().getName().contains(Physics2DGame.TYPE_SKY)
...
//                        Vector3f pos = selectedSprite.getWorldTranslation();
                        ...
                        Sprite sprite = game.getItem(selectedList.get(index));
                        Tile tile = new Tile(pos.x, pos.y, sprite.getWorldTranslation().z, marker.getLocalRotation().toAngleAxis(Vector3f.UNIT_Z), selectedList.get(index));
                        tile.setSpatial(sprite);
                        game.addTile(tile);
                        updateSelection();
            ...
            if (pickEvent.isKeyDown() && pickEvent.isRightButton() && !descriptionDialog.isVisible()
                    ...
                    && !pickEvent.getContactObject().getName().contains(Physics2DGame.TYPE_SKY)
                    && !pickEvent.getContactObject().getParent().equals(selectedSprite)
                    && pickEvent.getCursorPosition().x > (buttonSize * window.getScaleFactorWidth())
                    && pickEvent.getCursorPosition().x < window.getWidth() - (buttonSize * window.getScaleFactorWidth())
                    && pickEvent.getCursorPosition().y < window.getHeight() - (buttonSize * window.getScaleFactorHeight())) {
    ...
    public void drag(PickEvent pickEvent, float tpf) {
        if (isActive() && game != null && !descriptionDialog.isVisible()) {
            ...
            if (pickEvent.getContactPoint() != null
                ...
                marker.setLocalTranslation(pickEvent.getContactPoint().x, pickEvent.getContactPoint().y, 1);
                updateSelectedSpritePosition(pickEvent.getContactPoint().x, pickEvent.getContactPoint().y);
            }
