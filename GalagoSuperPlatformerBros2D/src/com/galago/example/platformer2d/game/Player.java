 ...
 * @author Nidebruyn
 */
public class Player extends Platform2DPlayer implements PhysicsCollisionListener, PhysicsTickListener {
    ...
    protected void init() {
        ...
        sprite = new Sprite(Platform2DGame.TYPE_PLAYER, 0.5f, 0.5f);
        sprite.setMaterial(game.getBaseApplication().getAssetManager().loadMaterial("Materials/player/ball.j3m"));
        sprite.setQueueBucket(RenderQueue.Bucket.Transparent);
        sprite.move(0, 0, 1f);

        rigidBodyControl = new RigidBodyControl(new CircleCollisionShape(0.25f), 1f);
            ...
            protected void controlUpdate(float tpf) {
                ...
                if (game.isStarted() && !game.isPaused() && !game.isGameOver()) {
                    ...
                    if (rigidBodyControl != null) {
                        ...
                        if (scale < 1f) {
                            scale += (tpf * 2f);
                            sprite.setLocalScale(1, scale, 1);
        ...
        Sprite face = new Sprite(Platform2DGame.TYPE_PLAYER, 0.45f, 0.45f);
