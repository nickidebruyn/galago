 ...
 * @author NideBruyn
 */
public class GlassControl extends AbstractControl {
    ...
    protected void controlUpdate(float tpf) {
        ...
        if (game.isStarted() && !game.isPaused() && !game.isGameOver()) {
            ...
            if (hitCount >= 1) {
                //Dispose the shape
                game.getBaseApplication().getEffectManager().doEffect("glass-break", spatial.getWorldTranslation().clone());
                game.getBaseApplication().getSoundManager().playSound("glass-break");
